...
export function Contact() {
  ...
  const handleSubmit = (event: Event) => {
    ...
    const message = `${t.contact.whatsappMessage}\nNombre: ${formData.name || t.contact.whatsappName}\nMensaje: ${formData.message || t.contact.whatsappNoMsg}`;
  ...
  return (
    <section className="py-24 md:py-32 bg-neutral-900" id="contact">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 lg:gap-20">
        <div>
          <h2 className="text-4xl sm:text-5xl font-black mb-10 sm:mb-12 uppercase italic">{t.contact.title}</h2>
        ...
        <div className="relative min-h-[280px] sm:min-h-[440px] border-4 border-white kinetic-border-blue">
          <div className="absolute inset-0 bg-neutral-950 flex items-center justify-center z-10">
            <div className="text-center">
              ...
              <p className="text-3xl font-black uppercase italic">{t.contact.locationTitle}</p>
              <p className="text-on-surface/60 font-bold mt-2">{t.contact.locationAddress}</p>
            </div>
          </div>
